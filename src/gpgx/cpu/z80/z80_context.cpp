...
#include "xee/mem/memory.h" // For Memcpy().
...
s32 Z80::LoadContext(u8* state)
{
  int bufferptr = 0;


  load_param(&m_pc, sizeof(m_pc));
  load_param(&m_sp, sizeof(m_sp));
  load_param(&m_af, sizeof(m_af));
  load_param(&m_bc, sizeof(m_bc));
  load_param(&m_de, sizeof(m_de));
  load_param(&m_hl, sizeof(m_hl));
  load_param(&m_ix, sizeof(m_ix));
  load_param(&m_iy, sizeof(m_iy));
  load_param(&m_wz, sizeof(m_wz));

  load_param(&m_af2, sizeof(m_af2));
  load_param(&m_bc2, sizeof(m_bc2));
  load_param(&m_de2, sizeof(m_de2));
  load_param(&m_hl2, sizeof(m_hl2));

  load_param(&m_r, sizeof(m_r));
  load_param(&m_r2, sizeof(m_r2));
  load_param(&m_iff1, sizeof(m_iff1));
  load_param(&m_iff2, sizeof(m_iff2));
  load_param(&m_halt, sizeof(m_halt));
  load_param(&m_im, sizeof(m_im));
  load_param(&m_i, sizeof(m_i));

  load_param(&m_nmi_state, sizeof(m_nmi_state));
  load_param(&m_irq_state, sizeof(m_irq_state));
  load_param(&m_after_ei, sizeof(m_after_ei));
  load_param(&m_cycles, sizeof(m_cycles));

  load_param(&m_last_fetch, sizeof(m_last_fetch));
  load_param(&m_EA, sizeof(m_EA));
...
s32 Z80::SaveContext(u8* state)
{
  int bufferptr = 0;

  save_param(&m_pc, sizeof(m_pc));
  save_param(&m_sp, sizeof(m_sp));
  save_param(&m_af, sizeof(m_af));
  save_param(&m_bc, sizeof(m_bc));
  save_param(&m_de, sizeof(m_de));
  save_param(&m_hl, sizeof(m_hl));
  save_param(&m_ix, sizeof(m_ix));
  save_param(&m_iy, sizeof(m_iy));
  save_param(&m_wz, sizeof(m_wz));

  save_param(&m_af2, sizeof(m_af2));
  save_param(&m_bc2, sizeof(m_bc2));
  save_param(&m_de2, sizeof(m_de2));
  save_param(&m_hl2, sizeof(m_hl2));

  save_param(&m_r, sizeof(m_r));
  save_param(&m_r2, sizeof(m_r2));
  save_param(&m_iff1, sizeof(m_iff1));
  save_param(&m_iff2, sizeof(m_iff2));
  save_param(&m_halt, sizeof(m_halt));
  save_param(&m_im, sizeof(m_im));
  save_param(&m_i, sizeof(m_i));

  save_param(&m_nmi_state, sizeof(m_nmi_state));
  save_param(&m_irq_state, sizeof(m_irq_state));
  save_param(&m_after_ei, sizeof(m_after_ei));
  save_param(&m_cycles, sizeof(m_cycles));

  save_param(&m_last_fetch, sizeof(m_last_fetch));
  save_param(&m_EA, sizeof(m_EA));
